...
def train_ld50_model():
    ...
    logger.info(f"LD50 MSE: {mean_squared_error(y_test, y_pred):.4f}")
    logger.info(f"LD50 R²: {r2_score(y_test, y_pred):.4f}")
...
def train_carcinogenicity_model():
    ...
    y_pred = model.predict(X_test_enc)
    logger.info(f"Carcinogenicity Accuracy: {accuracy_score(y_test, y_pred):.4f}")
    logger.info(f"Carcinogenicity F1 Score: {f1_score(y_test, y_pred):.4f}")
...
def train_general_tox_model():
    ...
    y_pred = model.predict(X_test_enc)
    logger.info(f"General Tox Accuracy: {accuracy_score(y_test, y_pred):.4f}")
    logger.info(f"General Tox F1 Score: {f1_score(y_test, y_pred):.4f}")
